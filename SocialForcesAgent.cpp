...
*/
	...
	if ( !_midTermPath.empty() )
	{
		...
		_WanderDirection = goalDirection;
	}
	else
	{
...
Util::Vector SocialForcesAgent::PursuitForce(float dt)
{	
	float FuturePosT;
	Util::Vector Desired_velocity;

	std::set<SteerLib::SpatialDatabaseItemPtr> _neighbors;
	getSimulationEngine()->getSpatialDatabase()->getItemsInRange(_neighbors,
		_position.x - (this->_radius + 2000),
		_position.x + (this->_radius + 2000),
		_position.z - (this->_radius + 2000),
		_position.z + (this->_radius + 2000),
		dynamic_cast<SteerLib::SpatialDatabaseItemPtr>(this));
	...
	{		
		...
		{
			tmp_agent = dynamic_cast<SteerLib::AgentInterface *>(*neighbour);
			if (tmp_agent->currentGoal().goalType == SteerLib::GOAL_TYPE_SEEK_STATIC_TARGET)
			{
				FuturePosT = (tmp_agent->position() - position()).length() / sf_max_speed;
				Desired_velocity = ((tmp_agent->position() + tmp_agent->velocity() * FuturePosT) - position()) * sf_max_speed;
			}
		}
	}
...
Util::Vector SocialForcesAgent::EvadeForce(float dt)
{
	float FuturePosT;
	Util::Vector Desired_velocity;
		...
		// for (int a =0; a < tmp_agents.size(); a++)
	{
		if ((*neighbour)->isAgent())
		{
			...
			{
				FuturePosT = (tmp_agent->position() - position()).length() / sf_max_speed;
			    Desired_velocity = (position() - (tmp_agent->position() + tmp_agent->velocity() * FuturePosT)) * sf_max_speed;
			}
		}
	}
	return Desired_velocity - velocity();
...
void SocialForcesAgent::updateAI(float timeStamp, float dt, unsigned int frameNumber)
{
	...
	_WanderDirection.x += 0.1 * (rand() % 3 - 1);
	_WanderDirection.z += 0.1 * (rand() % 3 - 1);
	_WanderDirection.y = 0.0f;
	_WanderDirection = normalize(_WanderDirection);
	goalDirection = _WanderDirection;
	Util::Vector WanderForce = (((goalDirection * PERFERED_SPEED) - velocity()) / (_SocialForcesParams.sf_acceleration / dt)) + velocity();
	...
	if (currentGoal().goalType == SteerLib::GOAL_TYPE_SEEK_DYNAMIC_TARGET)
	{
		Util::Vector PrefForce = (((goalDirection * PERFERED_SPEED) - velocity()) / (_SocialForcesParams.sf_acceleration / dt)) + velocity();
		Util::Vector repulsionForce = calcRepulsionForce(dt);
		Util::Vector proximityForce = calcProximityForce(dt);

		_velocity = PrefForce + repulsionForce + proximityForce + PursuitForce(dt);
		_velocity = clamp(velocity(), _SocialForcesParams.sf_max_speed);
		_velocity.y = 0.0f;
		_position = position() + (velocity() * dt);
	}
	if (currentGoal().goalType == SteerLib::GOAL_TYPE_FLEE_DYNAMIC_TARGET)
	{
		Util::Vector PrefForce = (((goalDirection * PERFERED_SPEED) - velocity()) / (_SocialForcesParams.sf_acceleration / dt)) + velocity();
		Util::Vector repulsionForce = calcRepulsionForce(dt);
		Util::Vector proximityForce = calcProximityForce(dt);

		_velocity = PrefForce + repulsionForce + proximityForce + EvadeForce(dt);
		_velocity = clamp(velocity(), _SocialForcesParams.sf_max_speed);
		_velocity.y = 0.0f;

		_position = position() + (velocity() * dt);
	}
	if (currentGoal().goalType == SteerLib::GOAL_TYPE_SEEK_STATIC_TARGET)
	{
		Util::Vector PrefForce = (((goalDirection * PERFERED_SPEED) - velocity()) / (_SocialForcesParams.sf_acceleration / dt)) + velocity();
		Util::Vector repulsionForce = calcRepulsionForce(dt);
		Util::Vector proximityForce = calcProximityForce(dt);

		_velocity = repulsionForce + proximityForce + WanderForce;
		_velocity = clamp(velocity(), _SocialForcesParams.sf_max_speed);
		_velocity.y = 0.0f;
		_position = position() + (velocity() * dt);
	}
